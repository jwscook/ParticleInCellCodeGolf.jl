...
v=collect(1:P.>P/2).*2 .-1.;r=zeros(N);X=copy(x);
E=zeros(3,N);V=copy(v);ik=2π*im*vcat(1,1:N/2,-N/2+1:-1);D=zeros(T,4);
...
@time @gif for t ∈1:T;X.=x;V.=v;F.*=NaN;
 ...
 scatter(x[1:P÷2],v[1:P÷2],c=1);scatter!(x[P÷2+1:P],v[P÷2+1:P],ylims=(-5,5),c=2);
...
xlabel!("Time");ylabel!("log 10");savefig("GaussianFixedPointQuietSimpson13.pdf")
