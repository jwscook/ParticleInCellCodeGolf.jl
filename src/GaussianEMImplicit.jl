using FFTW,LinearAlgebra,Plots,SpecialFunctions; let;
...
for i in 1:10;D[i,mod1.(i.+(-1:2),10)].=[1,-27,27,-1].*N/24;end
...
@gif for t ∈1:T;
  ...
  x.=X.+v[:,1]*dt; # next time step
  x.=mod.(x,1);p=sortperm(x);x=x[p];v=v[p,:];
  Cy.=By; Cz.=Bz;
  By.-=dt.*rif(-ik.*fft(Ez));
  Bz.-=dt.*rif(ik.*fft(Ey));
  by=(By.+Cy)./2
  ...
  for j∈1:P;
    θ=s(x[j]);
    a=sum(k->[Ex[k[1]],Ey[k[1]],Ez[k[1]]]*k[2],θ);
    v[j,:].+=a*dt/2
