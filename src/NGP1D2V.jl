using FFTW,LinearAlgebra,Plots,SpecialFunctions, StaticArrays, ProgressMeter
...
function pic()
  N=128;P=15N;T=2^14;TO=T÷16;
  n0=4*pi^2;vth=sqrt(n0)/N;dt=1/N/6vth;B0=sqrt(n0)/16;w=n0/P;
  @show N, P, T, TO, n0, vth, B0, dt
  @show 2pi^2 * (vth / B0)^2
  ξ=im*zeros(N);Es=zeros(N,TO);
  rx = halton.(0:P-1, 2, 1/sqrt(2));
  rvx = halton.(0:P-1, 3, 1/sqrt(2));
  rvy = halton.(0:P-1, 5, 1/sqrt(2));
  x = rx;#(bitreverse.(0:P-1).+2.0^63)/2.0^64;
  vx = vth * erfinv.(rvx);
  vy = vth * erfinv.(rvy);
  r=zeros(N);E=zeros(N);
  ik=2π*im*vcat(1,1:N/2,-N/2+1:-1);D=zeros(TO,5);
  f(g,c)=erf((g-c)*N)/2;ff(i,c)=f((i+0.5)/N,c)-f((i-0.5)/N,c);l=4eps();
  d(c)=((mod1(i,N),ff(i,c)) for i∈(-7:7).+Int(round(c*N)));
  ρ(x)=(r.*=0;for l∈d.(x);for k∈l;r[k[1]]+=k[2]*w;end;end;r);
